...
const createToken = (user) => {
  return jwt.sign(
    { ID_TK: user.ID_TK, Role: user.Role },
...
const registerUser = async (req, res) => {
  ...
  if (MatKhau.length < 3) {
    return res
      .status(400)
      .json({ success: false, error: "Mật khẩu phải có ít nhất 3 ký tự" });
  }
  ...
  try {
    ...
    const [existingPhone] = await conn.query(
      "SELECT * FROM TaiKhoan WHERE SDT = ?",
      [SDT]
    );
    if (existingPhone.length > 0) {
      return res
        .status(400)
        .json({ success: false, error: "Số điện thoại đã được sử dụng" });
    }
    ...
    if (Role === "admin") {
      await conn.query(
        "INSERT INTO QuanLy (ID_TK, TenQL, DiaChi) VALUES (?, ?, ?)",
        [userId, HoTen, DiaChi]
      );
    } else if (Role === "staff") {
      await conn.query(
        "INSERT INTO NhanVien (ID_TK, Ten_NV, DiaChi) VALUES (?, ?, ?)",
        [userId, HoTen, DiaChi]
      );
    } else if (Role === "user") {
      await conn.query(
        "INSERT INTO KhachHang (ID_TK, Ten_KH, DiaChi) VALUES (?, ?, ?)",
        [userId, HoTen, DiaChi]
      );
    }
...
const forgotPassword = async (req, res) => {
  ...
  const { Email, MatKhau, XacNhanMK } = req.body;
  ...
  try {
    ...
    const [user] = await conn.query("SELECT * FROM TaiKhoan WHERE Email = ?", [
      Email,
    ]);
    if (user.length === 0) {
    ...
    const hashedPassword = await bcrypt.hash(MatKhau, 10);
    await conn.query("UPDATE TaiKhoan SET MatKhau = ? WHERE Email = ?", [
      hashedPassword,
      Email,
    ]);
...
const updateUser = async (req, res) => {
  ...
  try {
    ...
    if (updateFields.length > 0) {
      const sql = `UPDATE TaiKhoan SET ${updateFields.join(
        ", "
      )} WHERE ID_TK = ?`;
    ...
    const [currentUser] = await conn.query(
      "SELECT Role FROM TaiKhoan WHERE ID_TK = ?",
      [id]
    );
    ...
    if (newRole === "admin" || currentRole === "admin") {
      const [existingAdmin] = await conn.query(
        "SELECT * FROM QuanLy WHERE ID_TK = ?",
        [id]
      );
      if (existingAdmin.length > 0 && HoTen && DiaChi) {
        await conn.query(
          "UPDATE QuanLy SET TenQL = ?, DiaChi = ? WHERE ID_TK = ?",
          [HoTen, DiaChi, id]
        );
      } else if (
        newRole === "admin" &&
        !existingAdmin.length &&
        HoTen &&
        DiaChi
      ) {
        await conn.query(
    ...
    if (newRole === "staff" || currentRole === "staff") {
      const [existingStaff] = await conn.query(
        "SELECT * FROM NhanVien WHERE ID_TK = ?",
        [id]
      );
      if (existingStaff.length > 0 && HoTen && DiaChi) {
        await conn.query(
          "UPDATE NhanVien SET Ten_NV = ?, DiaChi = ? WHERE ID_TK = ?",
          [HoTen, DiaChi, id]
        );
      } else if (
        newRole === "staff" &&
    ...
    if (newRole === "user" || currentRole === "user") {
      ...
      if (existingUser.length > 0 && HoTen && DiaChi) {
        await conn.query(
          "UPDATE KhachHang SET Ten_KH = ?, DiaChi = ? WHERE ID_TK = ?",
          [HoTen, DiaChi, id]
        );
      } else if (
        newRole === "user" &&
...
const loginUser = async (req, res) => {
  ...
  try {
    ...
    const [rows] = await conn.query("SELECT * FROM TaiKhoan WHERE Email = ?", [
      Email,
    ]);
    if (rows.length === 0) {
      return res
...
const getDashboardStats = async (req, res) => {
  ...
  try {
    ...
    const [pendingOrders] = await conn.query(
      "SELECT COUNT(*) as count FROM DonHang WHERE TrangThaiDonHang = 'Pending'"
    );
    const [deliveredOrders] = await conn.query(
      "SELECT COUNT(*) as count FROM DonHang WHERE TrangThaiDonHang = 'Delivered'"
    );
    const [cancelledOrders] = await conn.query(
      "SELECT COUNT(*) as count FROM DonHang WHERE TrangThaiDonHang = 'Cancelled'"
    );
    ...
    // Get total staff
    const [totalStaff] = await conn.query(
  ...
  try {
    ...
    if (Ten_KH || DiaChi) {
      ...
      if (updateKHFields.length > 0) {
        const sqlKH = `UPDATE KhachHang SET ${updateKHFields.join(
          ", "
        )} WHERE ID_KH = ?`;
    ...
    if (SDT) {
      ...
      const [khachHang] = await conn.query(
        "SELECT ID_TK FROM KhachHang WHERE ID_KH = ?",
        [id]
      );
      ...
      // Cập nhật SDT
      await conn.query("UPDATE TaiKhoan SET SDT = ? WHERE ID_TK = ?", [
        SDT,
        idTK,
      ]);
    }
  ...
  try {
    ...
    if (HoTen || DiaChi) {
      ...
      if (updateNVFields.length > 0) {
        const sqlNV = `UPDATE NhanVien SET ${updateNVFields.join(
          ", "
        )} WHERE ID_NV = ?`;
    ...
    if (SDT) {
      ...
      const [nhanVien] = await conn.query(
        "SELECT ID_TK FROM NhanVien WHERE ID_NV = ?",
        [id]
      );
...
const updatePassword = async (req, res) => {
  ...
  try {
    ...
    const [user] = await conn.query("SELECT * FROM TaiKhoan WHERE ID_TK = ?", [
      id,
    ]);
    ...
    // Cập nhật mật khẩu
    await conn.query("UPDATE TaiKhoan SET MatKhau = ? WHERE ID_TK = ?", [
      hashedPassword,
      id,
    ]);
